...
impl PolarsEngine {
    ...
    pub fn configure_columns(
        ...
        if let Some(c) = income   { self.income_col  = c; }
        if let Some(c) = expense  { self.expense_col = c; }
        if let Some(c) = date     { self.date_col    = c; }
    }
    ...
    pub fn load_csv_from_string(
        ...
        if line_count < 2 {
            return Err(AeraError::BadRequest("CSV en az bir başlık + bir veri satırı içermeli.".into()));
        }
        if line_count > 100_001 {
            return Err(AeraError::BadRequest("CSV 100.000 satır sınırını aştı.".into()));
        }
        ...
        for name in &col_names {
            if name.starts_with('=') || name.starts_with('+') || name.starts_with('@') {
                return Err(AeraError::BadRequest(
                    format!("Geçersiz sütun adı: '{}'. Formül karakteriyle başlayan isimler reddedilir.", name)
                ));
            }
        }
        ...
        tracing::info!(
            ...
            rows, cols, self.date_range.as_ref().map(|d| &d.days)
        );
    ...
    fn compute_date_range(&self, df: &DataFrame) -> Option<DateRange> {
        ...
        Some(DateRange { start, end, days: diff_days.max(1) })
    }
    ...
    pub fn column_sum(&self, column: &str) -> AeraResult<f64> {
        ...
        let series = df
            .column(column)
            .map_err(|_| AeraError::BadRequest(
                format!("Sütun bulunamadı: '{}'. Mevcut: {:?}",
                    column, df.get_column_names())
            ))?;

        let sum = series
            .cast(&DataType::Float64)
            .map_err(|e| AeraError::AgentExecutionError(
                format!("'{}' sayısal değil: {}", column, e)
            ))?
            .f64()
    ...
    fn compute_monthly_breakdown(&self) -> Vec<serde_json::Value> {
        ...
        let income_f64 = match df.column(&self.income_col)
            .and_then(|s| s.cast(&DataType::Float64))
        {
            Ok(s) => s,
            Err(_) => return vec![],
        };
        let expense_f64 = match df.column(&self.expense_col)
            .and_then(|s| s.cast(&DataType::Float64))
        {
            Ok(s) => s,
            Err(_) => return vec![],
        ...
        let mut monthly: std::collections::BTreeMap<String, (f64, f64)> = std::collections::BTreeMap::new();
    ...
    pub fn predict_cashflow(&self, months_ahead: usize) -> serde_json::Value {
        ...
        let incomes: Vec<f64>  = monthly.iter().map(|m| m["gelir"].as_f64().unwrap_or(0.0)).collect();
        let expenses: Vec<f64> = monthly.iter().map(|m| m["gider"].as_f64().unwrap_or(0.0)).collect();
        let nets: Vec<f64>     = monthly.iter().map(|m| m["net"].as_f64().unwrap_or(0.0)).collect();
        ...
        let residuals_net: Vec<f64> = nets.iter().enumerate().map(|(i, &n)| {
    ...
    pub fn health_score(&self) -> serde_json::Value {
        ...
        let months  = self.date_range.as_ref()
        ...
        let cash_score = piecewise_linear(cash_reserve_months, &[
            (-12.0, 0.0), (0.0, 2.0), (1.0, 8.0), (3.0, 18.0),
            (6.0, 30.0), (12.0, 38.0), (24.0, 40.0),
        ]).round() as u32;

        // Ratio için de benzer sıkılaştırma: %12 marjla A alan firma kalmasın.
        // 1.50 marj (%50 gelir > gider) → 30, 2.0 marj → tam puan.
        let ratio_score = piecewise_linear(ratio, &[
            (0.5, 0.0), (0.95, 3.0), (1.05, 8.0), (1.15, 16.0),
            (1.30, 24.0), (1.50, 32.0), (2.0, 35.0),
        ]).round() as u32;
    ...
    pub fn expense_by_category(&self) -> Vec<serde_json::Value> {
        ...
        for i in 0..n {
            ...
            if cat.is_empty() || cat == "null" { continue; }
            let exp = expense_f64.get(i).map(av_to_f64).unwrap_or(0.0);
        ...
        let mut result: Vec<serde_json::Value> = by_cat.iter()
            .filter(|(_, &v)| v > 0.0)
            .map(|(k, &v)| serde_json::json!({
                "kategori": k,
                "tutar":    round2(v),
    ...
    pub fn detect_anomalies(&self) -> serde_json::Value {
        ...
        let n = monthly.len();
        let history_gider: Vec<f64> = monthly[..n-1].iter()
            .map(|m| m["gider"].as_f64().unwrap_or(0.0)).collect();
        let history_gelir: Vec<f64> = monthly[..n-1].iter()
            .map(|m| m["gelir"].as_f64().unwrap_or(0.0)).collect();
        ...
        let z_gider = if std_gider > 0.0 { (last_gider - mean_gider) / std_gider } else { 0.0 };
        let z_gelir = if std_gelir > 0.0 { (last_gelir - mean_gelir) / std_gelir } else { 0.0 };
        ...
        if z_gider.abs() > 1.5 {
            let sev = if z_gider.abs() > 2.5 { "KRİTİK 🔴" } else { "YÜKSEK 🟠" };
            let dir = if z_gider > 0.0 { "anormal ARTIŞ" } else { "anormal DÜŞÜŞ" };
        ...
        if z_gelir.abs() > 1.5 {
            let sev = if z_gelir.abs() > 2.5 { "KRİTİK 🔴" } else { "YÜKSEK 🟠" };
            let dir = if z_gelir > 0.0 { "anormal ARTIŞ" } else { "anormal DÜŞÜŞ" };
            anomaliler.push(serde_json::json!({
                "tip": "Gelir Anomalisi",
                "ay": last_ay,
        ...
        let mesaj = if anomaliler.is_empty() {
            ...
            format!("{last_ay} ayında {} adet anomali tespit edildi!", anomaliler.len())
        };
        serde_json::json!({
    ...
    pub fn detect_cash_crunch(&self) -> serde_json::Value {
        ...
        let consecutive_negative = monthly.iter()
            .rev()
            .take_while(|m| m["net"].as_f64().unwrap_or(0.0) < 0.0)
            .count();
        let total_negative = monthly.iter()
            .filter(|m| m["net"].as_f64().unwrap_or(0.0) < 0.0)
            .count();
        let risk = if consecutive_negative >= 3 { "KRİTİK 🔴" }
            else if consecutive_negative >= 2 { "YÜKSEK 🟠" }
            else if consecutive_negative >= 1 { "ORTA 🟡" }
            else { "DÜŞÜK 🟢" };
        let uyari = if consecutive_negative >= 3 {
            format!("Son {} ay üst üste negatif nakit akışı! Acil aksiyon gerekiyor.", consecutive_negative)
        } else if consecutive_negative >= 2 {
            format!("Son {} ay negatif nakit akışı. Hızlı müdahale önerilir.", consecutive_negative)
        ...
        let son_aylar: Vec<serde_json::Value> = monthly.iter().rev().take(3).rev().cloned().collect();
        serde_json::json!({
    ...
    pub fn simulate_scenario(
        ...
        let sanitize = |v: f64| -> f64 {
            if !v.is_finite() { 0.0 } else { v.clamp(-1.0e12, 1.0e12) }
        };
        ...
        let base_income: f64  = monthly.iter().map(|m| m["gelir"].as_f64().unwrap_or(0.0)).sum::<f64>() / n;
        let base_expense: f64 = monthly.iter().map(|m| m["gider"].as_f64().unwrap_or(0.0)).sum::<f64>() / n;
        ...
        let hist_net: f64 = monthly.iter().map(|m| m["net"].as_f64().unwrap_or(0.0)).sum::<f64>();
        ...
        let runway_label = |exp: f64, inc: f64| -> &'static str {
            ...
            if runway < 1.0 { "KRİTİK 🔴" }
            else if runway < 3.0 { "YÜKSEK 🟠" }
            else if runway < 6.0 { "ORTA 🟡" }
            else { "DÜŞÜK 🟢" }
        };
...
fn stability_points(monthly: &[serde_json::Value]) -> f64 {
    ...
    let cov = if mean.abs() < 1.0 { 1.0 } else { (std / mean.abs()).min(1.0) };

    // Uzunluk bonusu da var — 12+ ay verisi yatırımcı için 3 aydan daha güvenilir.
    let length_bonus = piecewise_linear(n as f64, &[
...
fn holt_smooth(data: &[f64], alpha: f64, beta: f64) -> (f64, f64, Vec<f64>) {
    if data.is_empty() { return (0.0, 0.0, vec![]); }
    if data.len() == 1 { return (data[0], 0.0, vec![data[0]]); }
...
mod tests {
    ...
    fn test_risk_critical_when_high_burn() {
        ...
        assert!(risk.contains("KRİTİK"), "Yüksek giderde KRİTİK bekleniyor: {}", risk);
    }
    ...
    fn test_no_division_by_zero_when_balanced() {
        ...
        assert!(runway.contains("∞") || runway.contains("Pozitif"), "Dengeli durumda ∞ bekleniyor: {}", runway);
    }
    ...
    fn test_monthly_breakdown_groups_correctly() {
        ...
        let jan = breakdown.iter().find(|m| m["ay"] == "2026-01").expect("Ocak bulunamadı");
        let jan_gelir = jan["gelir"].as_f64().unwrap();
        assert!((jan_gelir - 80000.0).abs() < 1.0, "Ocak gelir 80000 olmalı: {}", jan_gelir);
    }
    ...
    fn test_health_score_returns_valid_range() {
        ...
        assert!(["A","B","C","D"].contains(&harf), "harf A/B/C/D olmalı: {}", harf);
    }
    ...
    fn test_expense_by_category_groups_correctly() {
        ...
        let maas = cats.iter().find(|c| c["kategori"] == "Maaş").expect("Maaş kategorisi olmalı");
        let kira = cats.iter().find(|c| c["kategori"] == "Kira").expect("Kira kategorisi olmalı");
    ...
    fn test_detect_cash_crunch_no_crunch() {
        ...
        assert!(risk.contains("DÜŞÜK"), "Pozitif akışta risk DÜŞÜK olmalı: {}", risk);
    }
    ...
    fn test_detect_cash_crunch_consecutive() {
        ...
        let risk = r["risk"].as_str().unwrap();
        assert!(risk.contains("YÜKSEK"), "2 ardışık negatif → YÜKSEK: {}", risk);
    }
    ...
    fn test_simulate_scenario_extra_expense() {
        ...
        assert!((sim_net - (-10000.0)).abs() < 1.0, "Ek 50K gider → -10K net bekleniyor: {}", sim_net);
    }
    ...
    fn test_simulate_scenario_extra_income_improves() {
        ...
        assert!(sim_net > 0.0, "Ek 40K gelir negatifi pozitife çevirmeli: {}", sim_net);
    }
    ...
    fn test_detect_anomalies_insufficient_data() {
        ...
        assert_eq!(anomali_sayisi, 0, "2 aylık veride anomali tespit edilmemeli");
    }
    ...
    fn test_detect_anomalies_normal_variation() {
        ...
        assert_eq!(anomali_sayisi, 0, "Normal varyasyonda anomali raporlanmamalı: {:?}", r);
    }
    ...
    fn test_detect_anomalies_spike() {
        ...
        let r = e.detect_anomalies();
        let anomali_sayisi = r["anomaliler"].as_array().map(|a| a.len()).unwrap_or(0);
        assert!(anomali_sayisi > 0, "Giderde büyük sıçramada anomali tespit edilmeli: {:?}", r);
    }
    ...
    fn test_predict_cashflow_has_confidence_interval() {
        ...
        let proj = r["projeksiyon"].as_array().expect("projeksiyon dizisi olmalı");
        ...
        assert!(proj[0]["ci_alt"].as_f64().unwrap() <= proj[0]["ci_ust"].as_f64().unwrap(), "ci_alt ≤ ci_ust");
    }
